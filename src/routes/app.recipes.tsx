...
export const Route = createFileRoute("/app/recipes")({
  head: () => ({
    meta: [
      { title: "Recipes — EgyCalorie" },
      { name: "description", content: "Lighter versions of Egyptian favourites." },
      { property: "og:title", content: "Recipes — EgyCalorie" },
...
function RecipesPage() {
  return (
    <div className="mx-auto max-w-6xl">
      <Panel title={t("Healthy recipes")} description={t("Egyptian dishes, rebalanced")}>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {recipes.map((r) => (
            <article key={r.id} className="overflow-hidden rounded-2xl border border-border">
              ...
              <div className="p-4">
                <h3 className="font-display text-sm font-extrabold">{t(r.title)}</h3>
